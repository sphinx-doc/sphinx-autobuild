...
def test_autobuild(mock_makedirs, mock_builder, mock_serve, mock_schedule):
    ...
    mock_serve.assert_called_once_with(host="127.0.0.1", root="/output", port=8000)
...
def test_autobuild_with_options(
    mock_makedirs, mock_watch, mock_builder, mock_serve, mock_schedule
):
    """
    ...
    # --port, --host
    mock_serve.assert_called_once_with(host="example.org", root="/output", port=8888)
